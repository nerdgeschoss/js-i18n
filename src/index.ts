...
export class Translator {
   ...
   */
  set translations(translations: TranslationEntry) {
    this._translations = translations;
  }

  /**
   * Gets the available locales. Currently this just returns
   * all keys of this._translations, e.g. ['de', 'en']
   */
  ...
  async setLocale(locale: Locale) {
    ...
    if (locale !== 'en') {
      try {
        ...
        console.warn(`Locale ${locale} does not exist in moment.js.`);
      }
    }
  ...
  translate(key: string, args: { [key: string]: string } = {}): string {
    let value = this._translations[this.locale];
    if (!value) {
